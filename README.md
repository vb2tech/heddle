...
├──────────────────────────┼──────────────────────────┤
│ TODO                     │ ACROSS SESSIONS          │
│ your durable backlog,    │ feed │ ready │ jobs      │
│ grouped by project       │                          │
└──────────────────────────┴──────────────────────────┘
```
...
### Todo — your backlog, not Claude's
...
- **Your todos** (bottom left) are durable, yours, span projects, and outlive
  every session.

Todos are grouped by project and dragged to reorder. Clicking the status dot
cycles open → doing → done.

The connective piece is **launch**: it opens a new terminal running `claude`
seeded with the todo's text, and then binds the todo to whichever session
appears. The session id cannot be known at launch time — the process has not
started yet — so the server adopts the first session to register in that
directory within five minutes. A bound todo shows that session's live
attention state and clicks through to it; `unlink` breaks the binding, and
nothing is adopted if you never launched anything.
...
Two halves of the same question. `ACROSS SESSIONS → ready` is what *Claude*
already planned and is unblocked. `TODO` is what *you* decided mattered.
Launching a todo hands it to a fresh session and keeps the link, so the backlog
shows what is in flight rather than going quiet the moment work starts.
...
| `TODO` | `⌘↵` in the box | Add a todo to the selected project |
| `TODO` | click the status dot | Cycle open → doing → done |
| `TODO` | drag a row | Reorder; the stored order is the list order |
| `TODO` | `launch` | Open a new terminal running `claude` seeded with it, and bind the session that starts |
| `TODO` | click the bound session | Inspect the session running that todo |
| `TODO` | `unlink` | Break a todo's binding to its session |
| `TODO` | `done` | Show or hide completed todos |
| `TODO` | `past prompts` → `queue` | Promote anything you have ever typed into a todo |
...
- Todos live in `data/todos.json` and session names in `data/labels.json`, both
  owned entirely by this app. Neither is ever written back into `~/.claude`.
- The todo **launch** action opens a *new* Terminal window running `claude`,
  seeded with the todo's text as the initial prompt.
